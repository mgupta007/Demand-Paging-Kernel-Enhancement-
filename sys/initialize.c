...
//	set_evec(14, pfintr);
    ...
    //    kprintf("flagssssss1s");
        set_evec(14, pfintr);


       initialize_Global();

  //    kprintf("flagsssssss22ss");
	//Allocate and initialize a page directory for NULL Process

       // set_evec(14, pfintr);
   

	Page_Directory(NULLPROC);
       
      // kprintf("flagssssssss333ssssss"); 
...
*/
	...
	*( (int *)pptr->pbase ) = MAGIC;
	...
	pptr->vmemlist = NULL;
	for (i = 0; i < 8; i++) {
		pptr->process_mapping[i].priv_flag = 0;
		pptr->process_mapping[i].bs_npages = 0;
		pptr->process_mapping[i].bs_pid = -1;
		pptr->process_mapping[i].process_in_bs = 0;
		pptr->process_mapping[i].bs_sem = -1;
		pptr->process_mapping[i].bs_status = BSM_UNMAPPED;
		pptr->process_mapping[i].bs_vpno = -1;
		pptr->process_mapping[i].total_number = 0;
	}
