...
/* emulate 386 "pushal" instruction */
	...
	*pushsp = pptr->pesp = (unsigned long)saddr;
	...
	int ii=0;
	do {
		pptr->process_mapping[ii].priv_flag = 0;
		pptr->process_mapping[ii].bs_npages = 0;
		pptr->process_mapping[ii].bs_pid = -1;
		pptr->process_mapping[ii].process_in_bs = 0;
		pptr->process_mapping[ii].bs_sem = -1;
		pptr->process_mapping[ii].bs_status = BSM_UNMAPPED;
		pptr->process_mapping[ii].bs_vpno = -1;
		pptr->process_mapping[ii].total_number = 0;
...
//	kprintf("Just before creating page directory for this process %d\n", pid);
	if (Page_Directory(pid) == -1) {
		kprintf("error in create");
		restore(ps);
		return SYSERR;
